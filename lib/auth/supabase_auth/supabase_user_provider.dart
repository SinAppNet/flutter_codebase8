...
class SinAppSupabaseUser extends BaseAuthUser {
  SinAppSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> sinAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SinAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
